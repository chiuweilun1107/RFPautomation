...
interface FooterSectionProps {
  footerData?: {
    default?: FooterData
    first_page?: FooterData
    even_page?: FooterData
  }
  pageNumber?: number
  isFirstPage?: boolean
  isEvenPage?: boolean
...
export function FooterSection({ footerData, pageNumber = 1, isFirstPage = false, isEvenPage = false }: FooterSectionProps) {
  if (!footerData) return null

  // 根據頁面類型選擇對應的頁尾數據
  let activeFooter: FooterData | undefined
  if (isFirstPage && footerData.first_page) {
    activeFooter = footerData.first_page
  } else if (isEvenPage && footerData.even_page) {
    activeFooter = footerData.even_page
  } else {
    activeFooter = footerData.default
  }

  if (!activeFooter || (!activeFooter.paragraphs?.length && !activeFooter.tables?.length)) {
    return null
  }
  ...
  return (
    <div className="footer-section border-t border-gray-200 dark:border-gray-700 pt-3 mt-4">
      {/* 渲染表格 */}
      {activeFooter.tables && activeFooter.tables.length > 0 && (
        <div className="footer-tables mb-2">
          {activeFooter.tables.map((table, tableIdx) => (
            <table key={tableIdx} className="w-full text-xs border-collapse">
              <tbody>
      ...
      {/* 渲染段落 */}
      {activeFooter.paragraphs && activeFooter.paragraphs.length > 0 && (
        <div className="footer-paragraphs text-xs text-gray-600 dark:text-gray-400">
          {activeFooter.paragraphs.map((para, idx) => (
