...
async function main() {
    try {
        ...
        console.log("🩹 Patching Insertion Logic...");

        // Correct the body parameters for the Supabase HTTP Request
        insertNode.parameters.bodyParameters = {
            parameters: [
                { name: "project_id", value: "={{ $json.projectId }}" },
                { name: "status", value: "todo" },  // FIXED: current_status -> status
                { name: "title", value: "={{ $json.tasks[0].title }}" }, // Now valid column
                { name: "description", value: "={{ $json.tasks[0].description }}" } // Add description too
            ]
        };
        ...
        if (updateRes.ok) {
            console.log("✅ SUCCESS: Fixed Column Mapping (status & title)!");
        } else {
